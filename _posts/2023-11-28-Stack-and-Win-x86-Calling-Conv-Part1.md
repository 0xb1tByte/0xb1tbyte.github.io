...
* `9` : هذا السطر يحوي أفكار مهمة وهي :
    ...
    ١ - نلاحظ أننا عرفنا `pointer variable` باسم `ptr` ونلاحظ أنه تم تعريفه في داخل الدالة ( `local variable` ) بالتالي سيتم تخزين الـ `ptr` في الـ `Stack` 

    ٢ - نلاحظ أيضًا أنه قمنا بحجز مساحة ( `two integers` ) في الذاكرة باستخدام الدالة `malloc`، المساحة التي تم حجزها ستكون في الـ `Heap` والمتغير `ptr` يُشير لها ( لكنّه مُخزّن في الـ `Stack` ) 
