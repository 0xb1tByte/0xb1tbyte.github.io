...
* `4` & `5` : نلاحظ أن المتغيرات `a` و `b` عبارة عن متغيرات تم تعريفها داخل الدالة `main` وليست من نوع `static` بالتالي هذه المتغيرات سنجدها في الـ `Stack`
* `6` : نلاحظ أن المتغير `y` تم تعريفه كـ `static` بالتالي سنجد هذا المتغير في الـ `BSS Segment`
...
* `12` : نلاحظ أننا أعطينا قيمة للعنصر الأول في الـ `array` ( `ptr[0]` ) القيمة `5` ، وسيتم تخزين الـ `ptr[0]` في الـ `Heap`  ( سيتم التخزين في الـ `Heap` لأن الـ `ptr` الذي قمنا بتعريفه في السطر رقم `9` يشير لمساحة في الـ `Heap`
